...
This is what you'd actually use to demo the engine live:
...
B-tree index, heap storage, Volcano-style executor, journaled transactions).
...
def format_rows(rows: list[dict]) -> str:
    ...
    widths = [max(len(str(c)), max((len(_cell(r.get(c))) for r in rows), default=0)) for c in cols]
    lines = []
    header = " | ".join(c.ljust(w) for c, w in zip(cols, widths))
    lines.append(header)
    lines.append("-+-".join("-" * w for w in widths))
    for r in rows:
        lines.append(" | ".join(_cell(r.get(c)).ljust(w) for c, w in zip(cols, widths)))
